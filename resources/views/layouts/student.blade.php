...
<link href="https://fonts.bunny.net/css?inter:400,600&display=swap" rel="stylesheet" />
...
<div class="min-h-screen bg-gray-50">
    ...
    <nav x-data="{ open: false, dropdownOpen: false }" class="bg-[#00563f] border-b border-[#003928] shadow-sm">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            {{-- Main H-Stack: Logo (Left) | Links (Center) | Dropdown/Hamburger (Right) --}}
            <div class="flex justify-between h-16">
                ...
                {{-- On small screens and up, this container expands (flex-1) and centers its content (justify-center) --}}
                <div class="hidden sm:flex sm:items-center sm:justify-center sm:flex-1">
                    <div class="space-x-8 sm:-my-px sm:flex">
                        {{-- Use theme="student" to apply green colors and the adjusted vertical padding for the underline --}}
                        <x-nav-link :href="route('student.home')" :active="request()->routeIs('student.home')" theme="student">
                            Home
                        </x-nav-link>
                        <x-nav-link :href="route('student.bookings.index')" :active="request()->routeIs('student.bookings.index')" theme="student">
                            My Bookings
                        </x-nav-link>
                        {{-- FIX: Updated the active check to match the plural route name 'student.feedbacks.index' --}}
                        <x-nav-link :href="route('student.feedbacks.index')" :active="request()->routeIs('student.feedbacks.index')" theme="student">
                            Give Feedback
                        </x-nav-link>
                    </div>
                </div>

                {{-- 3. Settings Dropdown (Right) & Hamburger --}}
        ...
        <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
            <div class="pt-2 pb-3 space-y-1">
                ...
                {{-- FIX: Updated the active check to match the plural route name 'student.feedbacks.index' --}}
                <x-responsive-nav-link :href="route('student.feedbacks.index')" :active="request()->routeIs('student.feedbacks.index')" theme="student">
                    Give Feedback
                </x-responsive-nav-link>
            </div>
